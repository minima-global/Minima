...
function checkRoundOneGame(blocknum, coin){
	...
	MDS.cmd(cmdstr,function(resp){
		...
		if(false && isplayer && age>20){
			...
			collectRoundOne(coin,address,playerpubkey);
...
function collectRoundOne(coin,address,pubkey){
	...
	MDS.log("COLLECT ROUND ONE AS PLAYER!");
	MDS.cmd(creator,function(resp){
		//MDS.log(JSON.stringify(resp));
	});
}

function lottoRoundOne(coin, pubkey, random){
