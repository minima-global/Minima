...
function writeFullLink(node,minidapp, install){
	...
	MDS.dapplink(minidapp.conf.name,function(link){
		...
		var visual 	 ='<a class="list-item-container" href="'+link.base+'" target="_blank" rel="noopener noreferrer">';
		...
		visual 		+='<div>'+minidapp.conf.description+'</div>';
...
function confirmDelete(uid){
	if (confirm("Are you sure ?")){
		...
		//Delete the MIniDAPP..
		MDS.cmd("mds action:uninstall uid:"+uid,function(resp){
			//Reload page
...
function confirmRead(uid){
	if (confirm("Are you sure READ?")){
		
		//Set READ permissions
		MDS.cmd("mds action:permission trust:read uid:"+uid,function(resp){
			//Reload page
			location.href = "index.html?uid="+MDS.minidappuid;	
		});
	}
...
function confirmWrite(uid){
	if (confirm("Are you sure WRITE?")){
		
		//Set READ permissions
		MDS.cmd("mds action:permission trust:write uid:"+uid,function(resp){
			//Reload page
			location.href = "index.html?uid="+MDS.minidappuid;	
		});
	}
