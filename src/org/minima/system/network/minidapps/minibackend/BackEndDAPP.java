...
public class BackEndDAPP extends MessageProcessor {
	 ...
	 */
	private String MINIMAJS="var MINIMA_MAIN_CALLBACK=null,MINIMA_MINIDAPP_CALLBACK=null,MINIMA_SERVER_LISTEN=[],MINIMA_USER_LISTEN=[],Minima={block:0,balance:{},txpow:{},logging:!1,init:function(n){n?MINIMA_MAIN_CALLBACK=n:Minima.log(\"No Main Minima Callback specified..\"),Minima.cmd(\"topblock;balance\",function(n){n[0].status&&(Minima.block=parseInt(n[0].response.txpow.header.block,10),Minima.txpow=n[0].response.txpow,n[1].status&&(Minima.balance=n[1].response.balance)),MinimaPostMessage(\"connected\",\"success\")})},log:function(n){java.lang.System.out.println(\"Service @ \"+(new Date).toLocaleString()+\" : \"+n)},notify:function(n,i){Minima.log(\"Notify : \"+n)},cmd:function(n,i){MinimaRPC(\"cmd\",n,i)},sql:function(n,i){MinimaRPC(\"sql\",n,i)},net:{onInbound:function(n,i){MINIMA_SERVER_LISTEN.push({port:n,callback:i})},start:function(n){MinimaRPC(\"net\",\"listen \"+n,null)},stop:function(n){MinimaRPC(\"net\",\"stop \"+n,null)},broadcast:function(n,i){MinimaRPC(\"net\",\"broadcast \"+n+\" \"+i,null)},onOutbound:function(n,i){MINIMA_USER_LISTEN.push({port:n,callback:i})},connect:function(n){MinimaRPC(\"net\",\"connect \"+n,null)},disconnect:function(n){MinimaRPC(\"net\",\"disconnect \"+n,null)},send:function(n,i){MinimaRPC(\"net\",\"send \"+n+\" \"+i,null)},info:function(){MinimaRPC(\"net\",\"info\",null)},stats:function(n){MinimaRPC(\"net\",\"stats\",n)},GET:function(n,i){MinimaRPC(\"net\",\"get \"+n,i)},POST:function(n,i,e){MinimaRPC(\"net\",\"post \"+n+\" \"+i,e)}},file:{save:function(n,i,e){MinimaRPC(\"file\",\"save \"+i+\" \"+n,e)},load:function(n,i){MinimaRPC(\"file\",\"load \"+n,i)},saveHEX:function(n,i,e){MinimaRPC(\"file\",\"savehex \"+i+\" \"+n,e)},loadHEX:function(n,i){MinimaRPC(\"file\",\"loadhex \"+n,i)},copy:function(n,i,e){MinimaRPC(\"file\",\"copy \"+n+\" \"+i,e)},move:function(n,i,e){MinimaRPC(\"file\",\"move \"+n+\" \"+i,e)},list:function(n,i){MinimaRPC(\"file\",\"list \"+n,i)},delete:function(n,i){MinimaRPC(\"file\",\"delete \"+n,i)}},minidapps:{list:function(n){Minima.cmd(\"minidapps list\",n)},listen:function(n){MINIMA_MINIDAPP_CALLBACK=n},send:function(n,i,e){Minima.cmd(\"minidapps post:\"+n+' \"'+i+'\"',e)},reply:function(n,i){replymsg={type:\"reply\",message:i,replyid:n},MinimaJSBridge.wspostreply(n,i)}},util:{getBalance:function(n){var i=Minima.balance.length;for(balloop=0;balloop<i;balloop++)if(Minima.balance[balloop].tokenid==n){var e=Minima.balance[balloop].confirmed,a=Minima.balance[balloop].sendable,t=Minima.balance[balloop].unconfirmed,o=Minima.balance[balloop].mempool;return\"0\"!==t||\"0\"!==o||a!==e?a+\" (\"+e+\") / \"+t+\" / \"+o:\"\"+e}return\"0\"},checkAllResponses:function(n){for(len=n.length,i=0;i<len;i++)if(1!=n[i].status)return Minima.log(\"ERROR in Multi-Command [\"+i+\"] \"+JSON.stringify(n[i],null,2)),!1;return!0},getStateVariable:function(n,i){var e=n.length;for(psloop=0;psloop<e;psloop++)if(n[psloop].port==i)return n[psloop].data;return null}}};function MinimaRPC(n,i,e){Minima.logging&&Minima.log(\"SERVICE_RPC : \"+n+\" \"+i),MinimaJSBridge.post(n,i,e)}function MinimaPostMessage(n,i){MINIMA_MAIN_CALLBACK&&MINIMA_MAIN_CALLBACK({event:n,info:i})}function MinimaBackEndListener(n){if(\"newblock\"==n.event)Minima.block=parseInt(n.txpow.header.block,10),Minima.txpow=n.txpow,MinimaPostMessage(\"newblock\",{txpow:n.txpow});else if(\"newtransaction\"==n.event){MinimaPostMessage(\"newtransaction\",{txpow:n.txpow,relevant:n.relevant})}else if(\"newtxpow\"==n.event){MinimaPostMessage(\"newtxpow\",{txpow:n.txpow})}else if(\"newbalance\"==n.event){Minima.balance=n.balance,MinimaPostMessage(\"newbalance\",{balance:n.balance})}else if(\"network\"==n.event)\"server_start\"==n.details.action||\"server_stop\"==n.details.action||\"server_error\"==n.details.action?sendCallback(MINIMA_SERVER_LISTEN,n.details.port,n.details):\"client_new\"==n.details.action||\"client_shut\"==n.details.action||\"message\"==n.details.action?n.details.outbound?sendCallback(MINIMA_USER_LISTEN,n.details.hostport,n.details):sendCallback(MINIMA_SERVER_LISTEN,n.details.port,n.details):\"post\"==n.details.action?MINIMA_MINIDAPP_CALLBACK?MINIMA_MINIDAPP_CALLBACK(n.details):Minima.minidapps.reply(n.details.replyid,\"ERROR - no minidapp interface found\"):Minima.log(\"UNKNOWN NETWORK EVENT : \"+evt.data);else if(\"txpowstart\"==n.event){MinimaPostMessage(\"miningstart\",{transaction:n.transaction})}else if(\"txpowend\"==n.event){MinimaPostMessage(\"miningstop\",{transaction:n.transaction})}}function sendCallback(n,e,a){var t=n.length;for(i=0;i<t;i++)if(n[i].port==e)return void n[i].callback(a)}";
	
	/**
	 * If it Crashes this is the counter.. 
	 */
	public int mCrashCounter = 0;
	
	/**
	 * Main BackEnd constructor.. 
	 * 
	 *  Init with JS but adds the DECIMAL JS class to every use
	 * 
	 * @param zScriptJS
	 * @param zMiniDAPPID
	...
	public int getCrashCounter() {
		return mCrashCounter;
	}
	
	public void incrementCrashCounter() {
		mCrashCounter++;
	}
