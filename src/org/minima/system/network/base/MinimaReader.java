...
import static org.minima.system.network.base.MinimaReader.MsgType.*;
    ...
    public void run() {
        try {
            ...
            while (!mNetClient.getSocket().isClosed()) {
				...
				} else if (msgtype == NETMESSAGE_INTRO || msgtype == NETMESSAGE_GREETING || msgtype == NETMESSAGE_TXPOWLIST) {
					if (len > MAX_INTRO) {
...
//				}
	  ...
	  public enum MsgType {
		...
		NETMESSAGE_INTRO(new MiniByte(1)),
		/**
		 * The peer has a new TXPOW. This is the ID. If the peer
		 * doesn't have it already he will request it
		 */
		NETMESSAGE_TXPOWID(new MiniByte(2)),
		/**
		 * Request the full details of a TXPOW. Either the complete
		 * txpow, or just the MMR proofs and updates.
		 */
		NETMESSAGE_TXPOW_REQUEST(new MiniByte(3)),
		/**
		 * Complete TXPOW. Only sent if the peer has requested it
		 */
		NETMESSAGE_TXPOW(new MiniByte(4)),
		/**
		 * A list of TxPoW details
		 */
		NETMESSAGE_TXPOWLIST(new MiniByte(5)),
		/**
		 * PING PONG
		 */
		NETMESSAGE_PING(new MiniByte(6)),
		/**
		 * GENERIC NETWORK MESSAGE
		 */
		NETMESSAGE_GENERIC(new MiniByte(7)),
		NETMESSAGE_P2P_RENDEZVOUS(new MiniByte(8)),
		NETMESSAGE_P2P_WALK_LINKS(new MiniByte(9)),

		NETMESSAGE_P2P_WALK_LINKS_RESPONSE(new MiniByte(10)),
		NETMESSAGE_P2P_SWAP_LINK(new MiniByte(11)),
		NETMESSAGE_P2P_DO_SWAP(new MiniByte(12)),
		NETMESSAGE_P2P_MAP_NETWORK(new MiniByte(13)),
		NETMESSAGE_P2P_GREETING(new MiniByte(14)),
		NETMESSAGE_P2P_NODE_NOT_ACCEPTING(new MiniByte(15)),
		UNKNOWN(new MiniByte());

		 private final MiniByte miniByte;

		MsgType(MiniByte miniByte) {
			this.miniByte = miniByte;
		}

		public MiniByte getMiniByte() {
			return miniByte;
		}

		static MsgType valueOf(MiniByte miniByte) {
			return Arrays.stream(values())
					.filter(x -> miniByte.equals(x.miniByte))
					.findFirst()
					.orElse(null);
		}
	}
