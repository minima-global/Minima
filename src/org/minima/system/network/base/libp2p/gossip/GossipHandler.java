...
package org.minima.system.network.base.libp2p.gossip;
...
import org.minima.system.network.base.LimitedSet;
...
public class GossipHandler implements Function<MessageApi, CompletableFuture<ValidationResult>> {
  ...
  private static final int GOSSIP_MAX_SIZE = 1024;
  ...
  private final Set<byte[]> processedMessages = LimitedSet.create(MAX_SENT_MESSAGES);
  ...
  public SafeFuture<ValidationResult> apply(final MessageApi message) {
    ...
    byte[] bytes = arr; // Bytes.wrap(arr);
    if (!processedMessages.add(bytes)) {
      // We've already seen this message, skip processing
      LOG.trace("Ignoring duplicate message for topic {}: {} bytes", topic, bytes.length);
      return VALIDATION_IGNORED;
    }
    LOG.trace("Received message for topic {}: {} bytes", topic, bytes.length);
  ...
  public void gossip(byte[] bytes) {
    if (!processedMessages.add(bytes)) {
      // We've already gossiped this data
      return;
    }

    LOG.trace("Gossiping {}: {} bytes", topic, bytes.length);
    SafeFuture.of(publisher.publish(Unpooled.wrappedBuffer(bytes), topic))
        .finish(
