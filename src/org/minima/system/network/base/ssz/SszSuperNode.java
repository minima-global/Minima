...
import org.minima.system.network.base.ssz.SszNodeTemplate.Location;
import org.minima.system.network.base.ssz.GIndexUtil.NodeRelation;
...
public class SszSuperNode implements TreeNode, LeafDataNode {
  ...
  private Bytes32 hashTreeRoot(int curDepth, int offset) {
    if (curDepth == depth) {
      ...
      return Bytes32.wrap((new Crypto()).hashSHA2(Bytes.wrap(
        hashTreeRoot(curDepth + 1, offset),
        hashTreeRoot(
            curDepth + 1,
            offset + elementTemplate.getSszLength() * (1 << ((depth - curDepth) - 1)))).toArray()));
