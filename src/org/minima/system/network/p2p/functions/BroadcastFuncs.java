...
public class BroadcastFuncs {
    ...
    public static List<Message> broadcastNodeNotAccepting(P2PState state, ArrayList<MinimaClient> clients) {
        ...
        return state.getOutLinksCopy().stream()
                .map(outLink -> {
                    Message sendMessage = new Message(P2P_SEND_MESSAGE);
                    return sendMessage
                            .addObject("client", getClientForInetAddress(outLink, clients, false))
                            .addObject("message", new Message(NETMESSAGE_P2P_NODE_NOT_ACCEPTING, sendMessage)
                                    .addObject("data", new P2PMsgNodeNotAccepting(state.getAddress(), sendMessage)));
                })
                .collect(toList());
    }
}
