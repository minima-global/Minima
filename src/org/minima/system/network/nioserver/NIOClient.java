...
public class NIOClient {
	...
	public NIOClient(String zHost, int zPort, MessageProcessor zMProcessor) {
		mUID 		= MiniFormat.createRandomString(8);
		mHost		= zHost;
		mPort		= zPort;
		mIncoming	= false;
		mMessageProcessor = zMProcessor;
	}
	
	public NIOClient(boolean zIncoming, String zHost, int zPort, SocketChannel zSocket, SelectionKey zKey, MessageProcessor zMProcessor) {
        mUID 		= MiniFormat.createRandomString(8);
 		...
 		if(mTraceON) {
 	   	...
 	   	while(mBufferIn.hasRemaining()) {
				...
				if(mReadCurrentPosition == mReadCurrentLimit) {
					//Post it !
					Message msg = new Message(NIOServer.NIOSERVER_NEWMSG);
					msg.addString("uid", mUID);
					msg.addObject("data", new MiniData(mReadData));
					mMessageProcessor.PostMessage(msg);
