...
public class ClientMessage implements Runnable {
	...
	public ClientMessage(String zClientUID, MiniData zData) {
	...
	public void run() {
		...
		try {
			...
			if(type.isEqual(MSG_GREETING)) {
				...
				//Send it
				ClientManager.sendNetworkMessage(mClientUID, MSG_IBD, ibd);
...
//				});
				...
				if(!exists) {
					//request it..
					ClientManager.sendNetworkMessage(mClientUID, MSG_TXPOWREQ, txpowid);
				}
				...
				if(txpow != null) {
					//request it..
					ClientManager.sendNetworkMessage(mClientUID, MSG_TXPOW, txpow);
				}else {
...
//				
				...
				if(valid) {
					...
					ClientManager.sendNetworkMessageAll(MSG_TXPOWID, txpow.getTxPoWIDData());
					...
					for(MiniData txn : txns) {
						...
						if(!exists) {
							...
							ClientManager.sendDelayedTxPoWReq(mClientUID, txn.to0xString(), txpow.getTxPoWID()+" missing txn");
						}
					}
					...
					if(!exists) {
						ClientManager.sendDelayedTxPoWReq(mClientUID, txpow.getParentID().to0xString(), txpow.getTxPoWID()+" missing parent");
					}
				...
				if(found) {
					...
					for(MiniData block : requestlist) {
						ClientManager.sendDelayedTxPoWReq(mClientUID, block.to0xString(), "PULSE");
					}
