...
public class MDSManager extends MessageProcessor {
	...
	private void doDefaultMiniHUB() throws Exception {
		...
		//Check for HUB
		checkInstalled("minihub", "minihub/minihub-0.5.7.mds.zip", allminis, true, true);
		
		//Do we Install the Default MiniDAPPs
		if(GeneralParams.DEFAULT_MINIDAPPS) {
		
			//Pending gets write permissions
			checkInstalled("pending", "default/pending-0.3.3.mds.zip", allminis, true);
			
			//Security MiniDAPP - backups / restore
			checkInstalled("security", "default/security-0.6.1.mds.zip", allminis, true);
			...
			checkInstalled("sql bench", "default/sqlbench-0.3.mds.zip", allminis, false);
