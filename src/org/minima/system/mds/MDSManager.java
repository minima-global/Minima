...
public class MDSManager extends MessageProcessor {
	...
	/**
	 * Have the MiniDAPPs been initialised..
	 */
	...
	protected void processMessage(Message zMessage) throws Exception {
		...
		if(zMessage.getMessageType().equals(MDS_INIT)) {
			...
			for(MiniDAPP dapp : dapps) {
				...
				addSessionID(sessionid, dapp.getUID());
			}
	...
	private void setupMiniDAPP(MiniDAPP zDAPP) {
		...
		String sessionid = MiniData.getRandomData(128).to0xString();
		addSessionID(sessionid, zDAPP.getUID());
	}
	
	private void addSessionID(String zSessionID, String zUID) {
		mSessionID.put(zSessionID, zUID);
	}
	...
	private void doDefaultMiniHUB() throws Exception {
		...
		String defminihub 	= udb.getDefaultMiniHUB();
		...
		checkInstalled("maxsolo", "default/maxsolo_2.4.0.mds.zip", allminis, false);
	...
	private void updateMiniHUB(String zMiniDAPPID) {
		...
		try {
			...
			//Which version..
			String version = jsonconf.getString("version");
			MinimaLogger.log("Updating default MiniHUB.. "+version);
