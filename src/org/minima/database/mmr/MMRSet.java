	...
	public MMRSet(MMRSet zParent, int zBitLength) {
		...
		if(mParent != null) {
			...
			for(MMREntry peak : peaks) {
				//Add the peak - hash only details..
				if(peak.getData().isHashOnly()) {
					setEntry(peak.getRow(), peak.getEntryNumber(), peak.getData());
				}else {
					MMRData pdata = peak.getData();
					setEntry(peak.getRow(), peak.getEntryNumber(), new MMRData(pdata.getFinalHash(),pdata.getValueSum()));
				}
...
//	}
	...
	public boolean checkProof(MMRProof zProof) {
		...
		if(proofset == null) {
			...
			MinimaLogger.log("checkProof Proof too Old current:"+getBlockTime()+" MAX:"+oldest.getBlockTime()+" proof:"+zProof);
			return false;
		}
