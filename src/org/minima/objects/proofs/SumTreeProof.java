...
public class SumTreeProof implements Streamable {

	public class STProofChunk {
		MiniData mHash;
		MiniNumber mValue;
		MiniByte mLeftRight;
		public STProofChunk(MiniByte zLeft, MiniData zHash, MiniNumber zValue) {
			mLeftRight = zLeft;
			mHash = zHash;
			mValue = zValue;
		}
	...
	protected ArrayList<STProofChunk> mProofChain;
...
//	}
	...
	public void addProofChunk(MiniByte zLeft, MiniData zHash, MiniNumber zValue) {
		mProofChain.add(new STProofChunk(zLeft, zHash, zValue));
	}
	...
	public STProofChunk getProofChunk(int zNum) {
	...
	public MiniData getChainSHAProof() {
		...
		try {
			...
			for(int i=0;i<len;i++){
				STProofChunk chunk = mProofChain.get(i);
	...
	public void readDataStream(DataInputStream zIn) throws IOException {
		...
		for(int i=0;i<len;i++) {
			...
			MiniNumber value = MiniNumber.ReadFromStream(zIn);
			
			mProofChain.add(new STProofChunk(left, hash, value));
		}
		
		finalizeHash();
	}
	
	public static SumTreeProof ReadFromStream(DataInputStream zIn){
		SumTreeProof proof = new SumTreeProof();
