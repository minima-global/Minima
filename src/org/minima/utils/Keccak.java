...
public class Keccak {
	...
	public static byte[] hashData(byte[] zData, int zBitLength){
		try {
			//Bouncy..
			Keccak keccak = new Keccak(zBitLength);
			keccak.update(zData, 0, zData.length);
			byte[] output = keccak.digestArray();
			return output;
		}catch(Exception exc) {
	...
	public static void main(String[] zArgs) {
		...
		//Speed test..
		MiniData data 	= MiniData.getRandomData(256);
		byte[] bytedata = data.getBytes();
		
		long timestart = System.currentTimeMillis();
		for(int i=0;i<mega;i++) {
			byte[] hashed = Crypto.getInstance().hashData(bytedata); 

		}
		
		long timeend 	= System.currentTimeMillis();
		long timediff 	= timeend - timestart;
		double mhs  	= 1000 / (double)timediff;
		System.out.println("Old hash : "+mhs+" mh/s");
		
		
		timestart = System.currentTimeMillis();
		for(int i=0;i<mega;i++) {
			byte[] output = Keccak.hashData(bytedata, 256); 
		}
		timeend 	= System.currentTimeMillis();
		timediff 	= timeend - timestart;
		mhs  		= 1000 / (double)timediff; 
		
		System.out.println("New hash : "+mhs+" mh/s");
