...
public class SHA256Tests {
    ...
    final static byte[][] macKeys = { BaseConverter.decode16("0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b"),
            BaseConverter.decode16("4a656665"), BaseConverter.decode16("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"),
    ...
    final static String[] macData = { "4869205468657265", "7768617420646f2079612077616e7420666f72206e6f7468696e673f",
            ...
            "cdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcd" + "cdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcd",
    ...
    final static String[] mac224 = { "b73d595a2ba9af815e9f2b4e53e78581ebd34a80b3bbaac4e702c4cc",
    ...
    final static String[] mac256 = { "9663d10c73ee294054dc9faf95647cb99731d12210ff7075fb3d3395abfb9821",
    ...
    public void testSHA256() {
        ...
        assertNotNull(i);
        SHA256Digest j = new SHA256Digest(i);
        // SHA256Digest k = new SHA256Digest(bytArray);
        assertNotNull(j);
        // assertNotNull(k);
        j.copy();
        j.getEncodedState();
        j.reset();
        assertNotNull(j.getAlgorithmName());
        System.out.println("String s = j.getAlgorithmName(); j:" + j + " resolves as " + j.getAlgorithmName());
        assertNotNull(i.getDigestSize());
        System.out.println("i.getDigestSize()); i:" + i + " resolves as " + i.getDigestSize());

        i.update(byt);
        assertNotNull(i.getDigestSize());
        System.out.println(
                "before j:" + j + " resolves as " + j.getByteLength() + " " + j.getDigestSize() + " " + bytArray);
        for (int x = 0; x < bytArray.length; x++) {
            System.out.print(bytArray[x] + " ");
        }
        j.update(bytArray, 1, 2);
        System.out.println("after j.update(bytArray, 2, 2) j:" + j + " resolves as " + j.getByteLength() + " "
                + j.getDigestSize() + " " + bytArray);
        assertNotNull(j);
        for (int x = 0; x < bytArray.length; x++) {
            System.out.print(bytArray[x] + " ");
        }
    ...
    public void testNewSHA256() {
        ...
        for (int j = 0; j != digests288.length; j++) {
            ...
            if (!Arrays.areEqual(b.decode16(digests288[j]), hash)) {
                assertNotNull(sha.getAlgorithmName());
                System.out.println("SHA256 mismatch on " + sha.getAlgorithmName() + " index " + sha);
            }
        }
        ...
        if (!Arrays.areEqual(b.decode16(digests288[messages.length]), hash)) {
            ...
            assertNotNull(sha.getAlgorithmName());
            System.out.println("SHA256 mismatch on " + sha.getAlgorithmName() + " 64k a");
        ...
        if (!Arrays.areEqual(b.decode16(digests288[messages.length] + 1), hash)) {
            ...
            assertNotNull(sha.getAlgorithmName());
            System.out.println("SHA256 mismatch on " + sha.getAlgorithmName() + " 64k alpha");
        } else if (Arrays.areEqual(b.decode16(digests288[messages.length] + 1), hash)) {
            System.out.println("k64-3 array is equal");
        }

        for (int n = 0; n != 64; n++) {
            sha.update(k64[n * 1024]);
            sha.update(k64, n * 1024 + 1, 1023);
        }
        ...
        if (!Arrays.areEqual(b.decode16(digests288[messages.length] + 1), hash)) {
            ...
            assertNotNull(sha.getAlgorithmName());
            System.out.println("SHA256 mismatch on " + sha.getAlgorithmName() + " 64k chunked alpha");
        } else if (Arrays.areEqual(b.decode16(digests288[messages.length] + 1), hash)) {
            System.out.println("k64-4 array is equal");
        }

        sha.doFinal(hash, 0);
        assertNotNull(sha.getByteLength());
        ...
        // sha.update(newHash, 0, newHash.length);
        // }
        // }

        // sha.doFinal(newHash, 0);

        // if (!Arrays.areEqual(newHash, hash)) {
        // assertNotNull(sha.getAlgorithmName());
        // System.out.println("SHA256 mismatch on " + sha.getAlgorithmName() + " extreme
        // data test");
        // }
    ...
    public void testSHA256DoFinal() {
        SHA256Digest sha = new SHA256Digest();

        byte[] hash = new byte[sha.getDigestSize()];
        sha.doFinal(hash, 0);

        for (int i = 0; i <= sha.getDigestSize(); ++i) {
            byte[] cmp = new byte[2 * sha.getDigestSize()];
            System.arraycopy(hash, 0, cmp, i, hash.length);

            byte[] buf = new byte[2 * sha.getDigestSize()];
            sha.doFinal(buf, i);

            if (!Arrays.areEqual(cmp, buf)) {
                assertNotNull(sha.getAlgorithmName());
                System.out.println("SHA256 offset doFinal on " + sha.getAlgorithmName());
            }
        }
    }
    ...
    // {
    // fail("SHA256 HMAC mismatch on " + digest.getAlgorithmName());
    // }
    // }
